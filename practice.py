...
def say_hi(name):
    """Prints "Hi" and then the user's name."""
...
def print_sign(int1):
    """Tells user if higher, lower or equal to zero"""
...
def is_divisible_by_three(int1):
    """Tells whether a number is divisible by three"""
...
def total_meal_price(meal,tip = 0.15):
    """Find the meal price with tip included (default @ 15%)"""
...
def sign_and_parity(int1):
    """Tells whether a number is positive/negative and even/odd"""
...
def full_title(name,job_title="Engineer"):
    """Returns full title and name of person. Defaults to Engineer if title not included"""

    full_title = job_title + " " + name
    return full_title

def add_new_number(int1,list1):
    """Adds a new number to the given list"""

    return list1.append(int1)
...
def write_letter(name,job_title,sender):
    """Returns beginning of letter addressing the person with job title. Ending with sender name"""
    
    full_name = full_title(name,job_title)
    return "Dear {}, I think you are amazing! Sincerely, {}".format(full_name,sender)
